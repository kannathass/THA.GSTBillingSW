    ...
    public partial class DeliveryNote : Form
    {
        ...
        private void DeliveryNote_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                textBoxTermsAndConditions.Text = customFieldSelection.GetTermsAndConditions(Convert.ToInt16(comboBoxCompany.SelectedValue), "Delivery Note");
            }
            bindCustomFields(customFieldSelection.GetCustomFields(Convert.ToInt16(comboBoxCompany.SelectedValue), "Delivery Note"));
            bindCustomFieldItems(customFieldSelection.GetCustomFields(Convert.ToInt16(comboBoxCompany.SelectedValue), "Delivery Note Items"));
        }
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please Provide Details!", "Delivery Note", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (ID != 0)
                {
                    if (comboBoxCompany.Text.Trim() != "" && comboBoxCustomer.Text.Trim() != "")
                    {
                        ...
                        {
                            deleteItem();
                            MessageBox.Show("Record Deleted Successfully", "Delivery Note", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            //DisplayData();
                            ClearData();
                        }
                    }
                    ...
                    {
                        MessageBox.Show("Please Select Details!", "Delivery Note", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
                else
                {
                    MessageBox.Show("No Invoice found!", "Delivery Note", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
        ...
        private void buttonCancelInvoice_Click(object sender, EventArgs e)
        {
            try
            {
                if (ID != 0)
                {
                    ...
                    {
                        CancelItem();
                        MessageBox.Show("Invoice canceled successfully", "Delivery Note", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        DisplayData();
                        ClearData();
                    }
                }
                ...
                if (dataGridViewList.Columns[e.ColumnIndex].Name == "Quantity")
                {
                        ...
                        )
                    {
                        MessageBox.Show(string.Format("You can enter Quantity upto '{0}' for the item '{1}'",
                            ...
                            , "Delivery Note", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        private void DisplayData()
        {
            ...
            using (SqlConnection con = new SqlConnection(conString))
            {
                ...
                cmd = new SqlCommand(@"SELECT [ID]
                  ...
                  ,[CF5]
                  ,[CF6]
              FROM [dbo].[TranDeliveryNote] where ID = @ID and Del_State=0", con);
                ...
                using (cmd)
                {
                    ...
                    FROM [TranDeliveryNoteItems] IVItem 
        ...
        private void setAutoSequenceNumber(string invoiceMode)
        {
            ...
            using (SqlConnection con = new SqlConnection(conString))
            {
                ...
                cmd.Parameters.AddWithValue("@DocumentType", "Delivery Note");
        ...
        {
            if (ConfigurationManager.AppSettings[string.Format("DeliveryNoteAutoComplete{0}", customFieldName)].ToString() == "1")
            {
                tbCustom.AutoCompleteCustomSource = customFieldSelection.GetCustomFieldData(customFieldName, "Delivery Note");
        ...
        private void fillItemInGridRow()
        {
            ...
            if (dialogResult == DialogResult.OK)
            {
                ...
                if (invoicewiseTaxCalc == "0")
                {
                    ...
                    //if DisableCreditNoteTaxStructure set as 0 then tax related detail will be available
                    if (ConfigurationManager.AppSettings["DisableCreditNoteTaxStructure"].ToString() == "0" || CompanyGSTN != string.Empty)
                    {
                        if (isIGSTApplicable == false)
                        {
        ...
        private void saveInvoiceItems()
        {
            using (SqlConnection con = new SqlConnection(conString))
            {
                ...
                for (int i = 0; i < dataGridViewList.Rows.Count - 1; i++)
                {
                    ...
                    cmd.Parameters.AddWithValue("@mode", "DeliveryNote-Insert");
        ...
        private Int32 saveInvoiceHeader()
        {
            ...
            using (SqlConnection con = new SqlConnection(conString))
            {
                ...
                if (ID == 0)
                {
                    cmd = new SqlCommand(@"INSERT INTO [dbo].[TranDeliveryNote]
           ([CompanyID]
           ,[NoteNumber]
           ,[NoteDate]
           ,[CustomerID]
     ...
     VALUES
           ...
           ,getDate())", con);
                ...
                else
                {
                    ...
                    if (ConfigurationManager.AppSettings["CompanywiseStock"].ToString() == "1")
                        ...
                        cmd.Parameters.AddWithValue("@StockCompanyID", 0);
                    cmd.Parameters.AddWithValue("@mode", "DeliveryNote-Reset");
                    using (cmd)
                    {
                        cmd.ExecuteNonQuery();
                    }

                    cmd = new SqlCommand(@"UPDATE [dbo].[TranDeliveryNote]
        ...
        private void deleteItem()
        {
            using (SqlConnection con = new SqlConnection(conString))
            {
                ...
                if (ConfigurationManager.AppSettings["CompanywiseStock"].ToString() == "1")
                    ...
                    cmd.Parameters.AddWithValue("@StockCompanyID", 0);
                cmd.Parameters.AddWithValue("@mode", "DeliveryNote-Delete");
                using (cmd)
                {
                    cmd.ExecuteNonQuery();
                }
        ...
        private void CancelItem()
        {
            ...
            {
                con.Open();
                cmd = new SqlCommand("Update [TranDeliveryNote] set NoteStatus='Canceled' Where ID=@ID ", con);
                cmd.Parameters.AddWithValue("@ID", ID);
                using (cmd)
                {
                    cmd.ExecuteNonQuery();
